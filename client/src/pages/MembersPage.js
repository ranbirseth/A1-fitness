...
const MembersPage = () => {
    ...
    const handleSubscriptionAction = async (action) => {
        if (!selectedMember || submittingRef.current)
            return;
        try {
            ...
            if (action === 'assign')
                await assignPlan(selectedMember._id, { planId: subFormData.planId, payment });
            else if (action === 'renew')
                await renewPlan(selectedMember._id, { planId: subFormData.planId, payment });
            else if (action === 'upgrade')
                await upgradePlan(selectedMember._id, { planId: subFormData.planId, payment });
