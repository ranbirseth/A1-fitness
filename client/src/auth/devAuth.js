...
export const DEV_BYPASS_ENABLED = import.meta.env.DEV && import.meta.env.VITE_DEV_AUTOLOGIN !== "off";
 ...
 */
export function tryDevAutoLogin() {
    ...
    if (attempted)
        return Promise.resolve(false);
    attempted = true;
    console.info(`%c[A1 Fitness] DEV AUTH BYPASS — signed in as ${DEV_CREDENTIALS.email} (${DEV_CREDENTIALS.role}). Disable with VITE_DEV_AUTOLOGIN=off in client/.env.development`, "color:#a78bfa;font-weight:bold");
    return login(DEV_CREDENTIALS)
        ...
        .catch(() => {
        console.warn("[DEV AUTH BYPASS] auto-login failed (server down or credentials changed). Falling back to the normal login page.");
        attempted = false;
        return false;
    });
}
