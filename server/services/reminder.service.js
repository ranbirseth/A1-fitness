...
async function sendReminderForMember(member, {
  ...
  const normalizedPhone = rawPhone ? whatsappService.normalizePhoneNumber(rawPhone) : null;
  ...
  if (!normalizedPhone) {
    result.whatsappStatus = "invalid_phone";
    result.whatsappReason = "phone number could not be normalized to E.164";
    dbg.error(`member ${member._id} -> whatsapp invalid_phone: raw number (${dbg.maskPhone(rawPhone)}) could not be normalized`);
    return result;
  }
  ...
  dbg.log(`member ${member._id} -> building template parameters (${parameters.length} placeholders) for template ${whatsappService.getTemplateName()}`);
...
async function sendRemindersToEligible({ gymId, branchCode, memberIds, now = new Date() } = {}) {
  ...
  summary.duplicatesSkipped = summary.inAppDuplicateSkipped;

  summary.whatsappStatus = whatsappService.isConfigured()
  ...
  dbg.log(
    `reminder.service completed -> eligible=${summary.eligible} inAppSent=${summary.inAppSent} whatsappSent=${summary.whatsappSent} whatsappFailed=${summary.whatsappFailed} whatsappInvalid=${summary.whatsappInvalid} whatsappNotConfigured=${summary.whatsappNotConfigured} whatsappSkipped=${summary.whatsappSkipped} whatsappStatus=${summary.whatsappStatus}${summary.truncated ? " (TRUNCATED)" : ""}`
  );
